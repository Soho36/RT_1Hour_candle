...
def hourly_engulf_signals(
    ...
    sr_level_columns = output_df.columns[8:]  # Assuming SR level columns start from the 8th column onwards
                                ...
                                if current_candle_close < current_sr_level:
                                    ...
                                    for subsequent_index in range(index + 1, len(output_df)):
                                        ...
                                        potential_ob_candle_center = (
                                                potential_ob_candle_high - (
                                                    (potential_ob_candle_high - potential_ob_candle_low) / 2)
                                        )
                                        ...
                                        if potential_ob_candle_center < current_sr_level:
                                            ...
                                            if green_candle_high - green_candle_low <= ob_candle_size:
                                                if potential_ob_doji >= 15:
                                                    ...
                                                    print(
                                                        f"Green candle is doji (has too small body {potential_ob_doji})%")
                                            else:
                                                print(f"Green candle is bigger than max size ({ob_candle_size})")
                                        else:
                                            print(
