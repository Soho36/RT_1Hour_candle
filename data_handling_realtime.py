...
nt8_sl_and_tp_signals_for_path = f'sl_and_tp_signal.txt'
...
def set_position_state_to_closed_before_start(state):  # Called from orders_sender.py
    ...
    with open(active_position_path, 'w', encoding='utf-8') as file:
        file.write(state)
        print(f"Active position is set to {state} before starting script")
...
def sl_and_tp_save_order_parameters_to_file(line_order_parameters):   # Called from orders_sender.py
    retries = 5
    for attempt in range(retries):
        try:
            with open(nt8_sl_and_tp_signals_for_path, 'w', encoding='utf-8') as file:
                file.writelines(line_order_parameters)
                print(f"NEW SL AND TP DATA IS SUCCESSFULLY SAVED TO FILE")
            break
        except PermissionError:
            print(f"Attempt {attempt + 1} failed. Retrying...")
            time.sleep(1)
